...
fossil fuels, rising gas emissions and advancing the global climate change crisis.
...
Consumer companies utilize plastic to deliver and package their products; these 
over-reliance and vast production of plastic has exacerbated this crisis. 

Companies that play a big role in the plastic pollution crisis like Coca-Cola 
and Nestle continue to implement flase solutions to the crisis by implementing 
faulty recyable products. Companies are avoiding making real change. Companies 
should commit to reduce waste and work alongside environmental organizations to 
...
When conducting this analysis, our team sought to understand the overarching 
question:
*Does the amount of plastic collected differ greatly between the United States 
and China, the two largest economies and polluters in the world?* 
We hypothesize that, given the highly different governmental and social 
structures in the United States versus China, these counties will differ in the 
amount of plastic they produce. We think that China will surpass the US in the 
amount of plastic collected in the recent years, as China is considered the 
...
different variables relating to plastic in these two countries. First we looked 
at the data regarding the average pollution from parent companies, then we 
examined whether type of plastic differs greatly between the counties, and 
lastly we sought to determine whether the year (2019 or 2020) made a difference 
in the amount of plastic collected, given that 2020 is a pandemic year versus a 
normal collection year in 2019. The three specific research questions we sought 
to explore are as follows:

This data is presented in terms of the amount of plastic collected from each 
parent company within each country. Thus, our 
**first research question is: Is 
...
countries, what is the difference in the types of plastic reported?** 
...
the 'other category' as these are less easily recycled.
...
the year 2019, we presume that the amount of plastic collected in this year as 
compared to 2020 will be significantly lower. 
...
Below is a visualization of the average amount of plastic collected per parent 
company in the US and China:
...
Below are three visualizations to determine the leading parent companies in each 
country as well as the top 10 parent companies globally:

```{r top-10-parent-companies-in-U.S, message = FALSE, warning = FALSE, echo=FALSE}
china_us_pollution %>%
  filter(country=="United States of America") %>% 
  filter(parent_company != "Grand Total") %>% 
  filter(parent_company != "Unbranded") %>% 
...
First, a hypothesis test was done to check the difference in means of these two 
countries for the average amount of plastic per country.
...
```{r rq-2-pivotlonger, message = FALSE, warning = FALSE, echo=FALSE,include=FALSE}
longer_data <- china_us_pollution %>%
  filter(parent_company != "Grand Total") %>%  #added
...
```{r rq-2-print, message = FALSE, warning = FALSE, echo=FALSE}
  print(longer_data) %>%
  kable
```

```{r ggplot research q 2, message = FALSE, warning = FALSE, echo=FALSE}
...
china_us_pollution %>% 
  ...
  geom_bar() + 
  labs(title = "Total Company Pollution in the United States and China in 2019", 
...
We also used a linear regression model to explore whether country is a 
...
in China compared to the United States of America. The intercept in our 
regression model is 19.17, meaning that if there was 
no 'country', we would expect the grand plastic total to be 19.17 units. 
In this context of this model, this intercept value is not applicable.
...
this is likely not the case as shown with recet trends of plastic pollution as 
...
up events because of the rise of the COVID-19 pandemic.

According to our linear regression model, the grand total amount of 
collected plastic would be equal to a count of 6.46 units if there is no 
recorded year; as a result, in this model, the intercept for our regression 
does not make sense and is not very applicable in this context.
If the year was 2020, we would expect the grand total amount to be 21.26
units higher than if it was 2019.  0.32% of the variance
in grand plastic total count is explained by the
year the plastic total was collected.

The year predictor was below the $\alpha$ = 0.05 level-- meaning that, based
on this linear regression model the relationship between
...
According to our multiple regression model, the grand total amount of collected
plastic would be equal to a count of 3.96 if there is no recorded year, 
no volunteering events and no volunteers; in this case the intercept for our
regression not very applicable in this context. For every increase in
...
None of the predictors were below the $\alpha$ = 0.05 level-- meaning we do 
...
According to our methodology, the difference in 
...
For Research Question 2, it is reasonable that "o" is the highest ranking column 
...
existing conclusions is important. Doing more background research
for the current project as well would have been helpful to organize
our hypotheses, methodology, and findings. I
